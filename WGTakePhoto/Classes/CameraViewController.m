...
@property (nonatomic,strong)ClipViewController *clipVC;
...
//    self.handleBlock = ^(NSString *location) {
//        Weakself.locaton = location;
//        if (Weakself.clipVC.MyhandleBlock) {
//            Weakself.clipVC.MyhandleBlock(location);
//        }
//    };
}
...
- (void)createdTool
{
    ...
    UILabel *titleLable = [[UILabel alloc] initWithFrame:CGRectMake((KWIDTH - 100)/2.0, 24, 100, 40)];
    titleLable.text = @"拍照";
    ...
    cameraBtn.frame = CGRectMake((KWIDTH - 50)/2.0, (120 - 50)/ 2.0, 65, 65);
    [cameraBtn setImage:[UIImage imageNamed:@"photo_take"] forState:UIControlStateNormal];
    ...
    headerBtn.frame =CGRectMake(10, ((120) - 20)/ 2.0, 50, 40);
    ...
    [self.toolView addSubview:headerBtn];
...
//    [self.toolView addSubview:photoBtn];
    ...
    lampBtn.frame = CGRectMake(KWIDTH-60 , ((120) - 30)/ 2.0, 50, 50);
    [lampBtn setImage:[UIImage imageNamed:@"photo_kai"] forState:UIControlStateSelected];
    [lampBtn setImage:[UIImage imageNamed:@"photo_kai"] forState:UIControlStateNormal];
...
-(void)jumpImageView:(NSData*)data{
    ...
    //viewController.location  =self.locaton;
...
//    UIImage *image = [UIImage imageWithData:data];
//    viewController.image = image;
//        [HudTool ShowLoading:@"定位中.." holding:NO];
//        [MapGerTool getGerdetailWithLoction:^(CLLocation *location, NSString *Str) {
//            if (Str.length>0) {
//                [HudTool hideHud];
//                viewController.location = Str;
//                viewController.picker = _imgPicker;
//                viewController.controller = self;
//                viewController.delegate = self;
//                viewController.isTakePhoto = YES;
//                [self presentViewController:viewController animated:NO completion:nil];
//            }
//            [MapGerTool getGerWithLoctiondetail:location.coordinate complete:^(NSString *str1, NSString *str2) {
//                //self.locaton = str2;
//                [HudTool hideHud];
//                viewController.location = str2;
//                viewController.picker = _imgPicker;
//                viewController.controller = self;
//                viewController.delegate = self;
//                viewController.isTakePhoto = YES;
...
- (void)openImagePickerControllerWithType:(UIImagePickerControllerSourceType)type
{
    ...
    _imgPicker.modalPresentationStyle = 0;
...
-(ClipViewController *)clipVC{
    if (!_clipVC) {
        _clipVC = [ClipViewController new];
    }
    return _clipVC;
}
